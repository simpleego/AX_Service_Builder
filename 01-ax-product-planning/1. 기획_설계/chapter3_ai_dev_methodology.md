...
```mermaid
flowchart LR
    A["3.1 설계<br/>무엇을 만들까"] --> B["3.2 작게 쪼개기<br/>한 번에 하나"]
    B --> C["3.3 명확한 지시<br/>컨텍스트 + 완료조건"]
    C --> D["3.4 매 단계 리뷰<br/>주의 깊은 수정"]
    D --> E["3.6 TDD<br/>테스트가 안전망"]
    E -->|다음 작업| B
    F["3.5 개발자의 판단력 Taste"] -.전 과정에 개입.-> A
    F -.-> D
    G["3.7 MCP / Skill / Hook"] -.자동화·확장.-> C
    G -.-> E
```
...
| **속임수(Cheating)** | 테스트를 비활성화·삭제하거나 기대값을 바꿔 통과시킴 | 즉시 거부, 테스트 보호 Hook 설정 (3.6 참조) |
...
| 절 | 한 줄 요약 |
|---|---|
| 3.1 | 바로 코딩하지 말고 탐색 → 계획 → 구현 → 커밋 |
| 3.2 | 한 번에 하나, 한 작업 = 한 테스트 = 한 커밋 |
| 3.3 | 컨텍스트는 가장 귀한 자원, 완료 조건은 실행 가능한 검사로 |
| 3.4 | 루프·범위 초과·테스트 조작을 감시하고, 증거로 리뷰하고, 과감히 되돌려라 |
| 3.5 | 코드는 싸졌고 판단력은 비싸졌다 |
| 3.6 | 비결정적 AI 위에 결정적 테스트를 세우고, 규칙은 Hook으로 강제 |
| 3.7 | MCP는 연결, Skill은 능력, Hook은 강제 – 최소한으로, 목적에 맞게 |
